...
**Newsletters** ([`templates/my-account-v2-demo/newsletters.php`](../includes/plugins/woocommerce/my-account/templates/my-account-v2-demo/newsletters.php) · [`src/my-account-v2-demo/newsletters.js`](../src/my-account-v2-demo/newsletters.js)). Sectioned list (Featured / Technology / Subscriber-only) of newsletter rows. Each row's *Sign up* / *Unsubscribe* button toggles the row's subscribed state in the DOM and fires a snackbar — no fetch; subscription state persists via `localStorage` (key `newspack-my-account-v2-demo:newsletters`) so a reload keeps the toggles you flipped. The bottom *Unsubscribe from all* button cascades the toggle and disables itself once nothing is subscribed. `?my-account-v2-demo=no-categories` flattens the sections.
...
└── templates/my-account-v2-demo/
    ...
    ├── account-settings.php                  # Phase 9 — account settings (profile + password + delete)
    └── partials/
        ├── newsletters-row.php
...
├── newsletters.js / donations.js / subscriptions.js / payment-methods.js / account-settings.js
...
| `wc_get_template` | 2 | swap of `myaccount/form-edit-account.php` (Phase 9) | Returns the v2-demo `account-settings.php` path when the demo is active. Priority 2 lands after v1's priority-1 swap of the same template, so v1 stays untouched on non-demo `/edit-account/` |
...
| `wp_footer` | 100 | `render_homepage_overlay` | Renders the homepage right-side drawer overlay markup when the same `?my-account-v2-demo` flag is present on a non-`/my-account/` URL. Body-class scope is its own (`newspack-my-account-v2-demo-homepage`) so styles don't leak |
...
- Remove the `newspack-my-account-v2-demo` body-class scope in [`style.scss`](../src/my-account-v2-demo/style.scss). The wrapper is currently load-bearing — it scopes the small handful of scoped rules each documented inline — but if those rules fold cleanly into v1's `_my-account.scss`, the wrapper goes too.
