...
import { CheckboxControl as WpCheckboxControl, MenuItem, Tooltip } from '@wordpress/components';
...
const getEndpointLabel = ( endpoint: Endpoint ) => {
...
const getRequestStatusIcon = ( status: 'pending' | 'finished' | 'killed' ) => {
	const icons = {
...
const hasEndpointErrors = ( endpoint: Endpoint ) => {
...
const Webhooks = () => {
	...
	const [ deleting, setDeleting ] = useState< false | Endpoint >( false );
	const [ toggling, setToggling ] = useState< false | Endpoint >( false );
	const [ viewing, setViewing ] = useState< false | Endpoint >( false );
	const [ editing, setEditing ] = useState< false | WebhookEditingState >( false );
	const [ editingError, setEditingError ] = useState< false | { message: string } >( false );

	const fetchEndpoints = () => {
		setInFlight( true );
		apiFetch< Endpoint[] >( { path: '/newspack/v1/webhooks/endpoints' } )
			.then( response => {
	...
	const upsertEndpoint = ( endpoint: Partial< Endpoint > ) => {
		setInFlight( true );
	...
	>( false );
	const [ testError, setTestError ] = useState< false | { message: string } >( false );
	const sendTestRequest = ( url: string | undefined, bearer_token: string | undefined ) => {
		setInFlight( true );
		setTestError( false );
		setTestResponse( false );
		apiFetch< { success: boolean; code: number; message: string } >( {
