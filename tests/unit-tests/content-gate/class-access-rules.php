...
class Newspack_Test_Access_Rules extends WP_UnitTestCase {
	...
	/**
	 * Test normalize_rules converts flat rules to OR groups (one group per rule).
	 */
	public function test_normalize_rules_flat_to_or_groups() {
		$flat_rules = [
			[
				'slug'  => 'rule_a',
				'value' => '1',
		...
		$normalized = Access_Rules::normalize_rules( $flat_rules );

		$this->assertCount( 2, $normalized, 'Each flat rule should become its own group.' );
		$this->assertEquals(
			[
				[
					'slug'  => 'rule_a',
	...
	/**
	 * Test flat rules use OR logic: passing any single rule grants access.
	 */
	public function test_flat_rules_use_or_logic() {
		// Register two test rules: one always passes, one always fails.
		Access_Rules::register_rule(
			[
				'id'       => 'always_pass',
				'name'     => 'Always pass',
		...
		// Flat rules: one passes, one fails. With OR logic, access should be granted.
		$flat_rules = [
			[
				'slug'  => 'always_fail',
				'value' => null,
		...
		$this->assertTrue(
			Access_Rules::evaluate_rules( $flat_rules, self::$member_user_id ),
			'Flat rules should use OR logic: passing any rule should grant access.'
		);

		// Both fail: access should be denied.
		$both_fail = [
			[
				'slug'  => 'always_fail',
				'value' => null,
		...
		$this->assertFalse(
			Access_Rules::evaluate_rules( $both_fail, self::$member_user_id ),
			'Access should be denied when all flat rules fail.'
		);
	}
