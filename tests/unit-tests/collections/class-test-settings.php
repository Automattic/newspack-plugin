...
class Test_Settings extends WP_UnitTestCase {
	...
	/**
	 * Test get_custom_names returns default values when custom naming is disabled.
	 *
	 * @covers \Newspack\Collections\Settings::get_custom_names
	 */
	public function test_get_custom_names_returns_defaults_when_disabled() {
		$names = Settings::get_custom_names();
		$this->assertEquals( 'Collections', $names['name'] );
		$this->assertEquals( 'Collection', $names['singular_name'] );
		$this->assertEquals( 'collection', $names['slug'] );
	}

	/**
	 * Test get_custom_names returns custom values when enabled.
	 *
	 * @covers \Newspack\Collections\Settings::get_custom_names
	 */
	public function test_get_custom_names_returns_custom_values_when_enabled() {
		$custom_name          = 'Issues';
		...
		$names = Settings::get_custom_names();
		$this->assertEquals( $custom_name, $names['name'] );
		$this->assertEquals( $custom_singular_name, $names['singular_name'] );
		$this->assertEquals( $custom_slug, $names['slug'] );
	}

	/**
	 * Test get_custom_names with custom parameters overrides settings.
	 *
	 * @covers \Newspack\Collections\Settings::get_custom_names
	 */
	public function test_get_custom_names_with_custom_parameters() {
		$custom_name          = 'Issues';
		$custom_singular_name = 'Issue';
		$custom_slug          = 'issue';

		$names = Settings::get_custom_names( $custom_name, $custom_singular_name, $custom_slug );
		$this->assertEquals( $custom_name, $names['name'] );
		$this->assertEquals( $custom_singular_name, $names['singular_name'] );
		$this->assertEquals( $custom_slug, $names['slug'] );
	}
	...
	/**
	 * Test get_custom_names handles empty custom values gracefully.
	 *
	 * @covers \Newspack\Collections\Settings::get_custom_names
	 */
	public function test_get_custom_names_handles_empty_custom_values() {
		$custom_settings = [
			'custom_naming_enabled' => true,
			'custom_name'           => '',
