...
class Network_Wizard extends Wizard {
	...
	public function __construct() {
		...
		// Remove "Node Settings" from the menu.
		remove_action( 'admin_menu', [ \Newspack_Network\Node\Settings::class, 'add_menu' ] );
	...
	/**
	 * Called from parent constructor 'admin_menu'.
	 */
	public function add_page() {

		// Move the entire menu.  See function's docblock for notes.
		$this->move_menu();

		if( 'node' === $this->get_site_role() ) {
			
			if ( is_callable( [ \Newspack_Network\Node\Settings::class, 'render' ] ) ) {
			
				// Re-add "Node settings" as hidden
				add_submenu_page(
					'', // 'newspack-network',
					__( 'Node Settings', 'newspack-plugin' ),
					__( 'Node Settings', 'newspack-plugin' ),
					'manage_options', // copied from original callback.
					'newspack-network-node',
					[ \Newspack_Network\Node\Settings::class, 'render' ]
				);
				
				// @todo Need to force the main menu to be Active when on this tab...
				// also, maybe stop wordpress from condensing the menu when only one item is it...
				// OR just don't do tabs for is_node ?????
	...
	public function enqueue_scripts_and_styles() {
		// Dont load parent.  Scripts and styles are enqueued by Admin Header instead.
	}
	 ...
	 * Move entire menu.
	 * 
	 * The code below will move the entire $menu instead of overriding the different add_menu_page/add_submenu_page functions.
