...
<form method="post" action="<?php echo self_admin_url( 'index.php?page=newspack-subscriptions-wizard' ); ?>">
	...
	<input type="text" name="name" placeholder="What is this product called? e.g. Valued Donor" value="<?php echo esc_attr( $product_name ); ?>" />
	...
	<div class="price_fields">
		...
		<select name="subscription_frequency" class="subscription_frequency_input">
	...
	<div>
		...
		<input type="checkbox" name="choose_price" class="choose_price_input" value="1" <?php checked( $choose_price ); ?> /> Name your price
	</div>
	<div class="choose_price_fields">
		<div>
		...
		<div>
			...
			<input type="checkbox" name="hide_minimum_price" value="1" <?php checked( $hide_min_price ); ?> /> Hide minimum price
		</div>
	</div>
...
(function($){
	function update_form() {
		if ( $( '.choose_price_input').is( ':checked' ) ) {
			$( '.price_fields' ).find( 'input, select' ).attr( 'disabled', true );
			$( '.choose_price_fields' ).show();
			$( '.choose_price_fields .subscription_frequency_input' ).attr( 'disabled', false );
		} else {
			$( '.price_fields' ).find( 'input, select' ).attr( 'disabled', false );
			$( '.choose_price_fields' ).hide();
			$( '.choose_price_fields .subscription_frequency_input' ).attr( 'disabled', true );
		}
	};
	update_form();
	$( '.choose_price_input' ).on( 'change', update_form );
})(jQuery)
</script>