...
class WooCommerce_Subscriptions {
	...
	/**
	 * Flag for bypassing payment retry checks.
	 *
	 * @var bool
	 */
	 ...
	 * : Produce more output.
	 *
	 * [--bypass-payment-retry-checks]
	 * : Bypass payment retry and update the subscription status to expired.
	 *
	 * [--ids]
	...
	public function migrate_expired_subscriptions( $args, $assoc_args ) {
		...
		self::$bypass  = isset( $assoc_args['bypass-payment-retry-checks'] ) ? true : false;
		$scheduled     = 0;
		$updated       = 0;
		$page          = 1;
		...
		while ( ! empty( $subscriptions ) ) {
			foreach ( $subscriptions as $subscription ) {
				...
				$is_supported = ! $subscription->is_manual() && $subscription->payment_method_supports( 'subscription_date_changes' );
				$last_retry   = null;
				if ( ! self::$bypass ) {
					if ( ! $is_supported ) {
						if ( self::$verbose ) {
				...
				$retry_date       = $last_retry ? $last_retry->get_date() : $subscription->get_date_modified();
				...
				if ( $is_supported && wcs_date_to_time( $retry_date ) + ( $on_hold_duration * DAY_IN_SECONDS ) > time() ) {
					if ( self::$verbose ) {
