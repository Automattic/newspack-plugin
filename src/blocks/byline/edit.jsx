...
import {
   ...
   */
  const parseForPreview = (metaByline, showAvatar = false, avatarSize = 24, linkToAuthorArchive = true, authors) => {
    if (!metaByline) { return ''; }
    ...
    return metaByline.replace(
      ...
      (match, id, name) => {
        ...
        if (showAvatar && avatarUrl) {
          avatar = `<span class="newspack-byline-avatar" style="display: inline-block; margin-right: 8px; vertical-align: middle;">
            <img 
              src="${avatarUrl}"
              alt="${name}" 
  ...
  const Edit = ({ attributes, context, setAttributes, isSelected }) => {
    ...
    useEffect(() => {
      if (!customByline && authors.length > 0) {
        let defaultByline = 'By ';
      ...
      authorTokens.forEach(token => {
        ...
        if (matchedAuthor) {
          ...
          if (avatarUrl) {
            ...
            avatarEl.className = `avatar-display avatar-display-${authorId}`;
    ...
    }, [setAttributes, showAvatar, avatarSize, authors]);
    ...
    const insertToken = (token) => {
      ...
      // Create token element with additional data-name attribute
      ...
      tokenElement.dataset.name = token.display_name; // Add name to dataset for reliability
      ...
      if (showAvatar && token) {
        ...
        if (avatarUrl) {
          const avatarEl = document.createElement('span');
          avatarEl.className = `avatar-display avatar-display-${token.id}`;
          avatarEl.style.cssText = `
            display: inline-block;
            margin-right: 4px;
            vertical-align: middle;
        ...
        <div {...blockProps}>
          ...
          {isSelected ? (
            ...
            <div 
              ...
              dangerouslySetInnerHTML={{ __html: parseForPreview(customByline, showAvatar, avatarSize, linkToAuthorArchive, authors) }} 
            />
          )}
        </div>
      </>
